...
interface V4Contracts {
  poolManager: Address; // Uniswap v4 PoolManager contract
  positionManager: Address; // Uniswap v4 PositionManager (NFT positions)
  quoter: Address; // Uniswap v4 Quoter (swap simulation)
  stateView: Address; // Uniswap v4 StateView (pool state queries)
...
Represents a fully initialized Uniswap v4 pool with current on-chain state. Key properties:
...
interface BuildAddLiquidityArgs {
  pool: Pool; // Uniswap v4 SDK Pool instance
...
interface GetPositionResponse {
  position: Position; // Uniswap v4 SDK Position instance
